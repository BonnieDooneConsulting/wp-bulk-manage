...
class wp_bulk_manage_plugin {
	...
	public function admin_initialize(): void {
		register_setting( $this->base->config_name . '_group', $this->base->config_name,
			array( $this->base, '_sanitize_option_input' ) );
		register_setting( $this->base->users_config_name . '_group', $this->base->users_config_name,
			array( $this->base, '_sanitize_option_input' ) );
	}
	...
	public function _admin_enqueue( $hook ): void {
		...
		if ( in_array( $hook, $hooks ) ) {
			wp_enqueue_script( 'wp_bulk_manage_users_settings',
				plugin_dir_url( __FILE__ ) . 'assets/js/bulk-manage-users.js', array( 'jquery' ) );
			wp_localize_script( 'wp_bulk_manage_users_settings', 'wp_bulk_manage',
